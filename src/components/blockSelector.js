...
const BlockSelector = observer(class BlockSelector extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    _onContextMenuChange = async item => {
        ...
        const currentBlock = store.blocksForCurrentPage.find(block => block.id === blockId)
        
        await store.updateBlock(blockId, currentBlock?.content || '', item.id, item.data)
        this.props.onFocusChild && this.props.onFocusChild(blockId)
    }

    _onReturnKeyPressed = _ => this.props.onReturnKeyPressed && this.props.onReturnKeyPressed()
    _onDeleteKeyPressed = _ => this.props.onDeleteKeyPressed && this.props.onDeleteKeyPressed()
    ...
    _openMenuHandleClick = (e) => {
        const boundingBox = this.contextMenuHandleRef.current?.getBoundingClientRect() || { height: 0, width: 0 }
        this.blockContextMenuRef.current?.open(boundingBox.top + boundingBox.height, boundingBox.left)//this.contextMenuHandleRef.current?.offsetLeft)
    }
    ...
    _onBlockContextMenuClick (args) {
        ...
        switch(action) {
            ...
            case 'setEmailType':
                ...
                break
            default:
                this.props.onBlockContextMenuClick && this.props.onBlockContextMenuClick({ data, action }) 
        }
     }
    ...
    _setMenuItemAccess = e => {
        ...
        if (e.parentItem === null) {
            const enabledItems = e.items.filter(data => data.enable).map(data => data.text)
            const disabledItems = e.items.filter(data => !data.enable).map(data => data.text)

            if (this.blockContextMenuRef.current) {
                if(enabledItems.length > 0) this.blockContextMenuRef.current?.enableItems(enabledItems, true)
                if(disabledItems.length > 0) this.blockContextMenuRef.current?.enableItems(disabledItems, false )
            }
        }
    ...
    componentDidUpdate () {
        if (this.props.showBlockTypeSelector && store.focusedBlockId === this.props.blockId) {
    ...
    _getContextMenuHandleIcon = _ => this.props.showBlockTypeSelector ? '+' : '#'
    _getControls = _ => BlockSelectorControls.map(ctrl => ({ ...ctrl, control:undefined }))
       
    render () {                
        const Menu = <>
            <div id={this.menuId} ref={this.contextMenuHandleRef}
                className={`menuHandle ${this.props.isHover || (true || store.focusedBlockId === this.props.blockId) ? 'show' : ''}`}>
                <i className="fa-solid fa-ellipsis-vertical" onClick={this._openMenuHandleClick}></i>{this._getContextMenuHandleIcon()}
            </div>
         </>

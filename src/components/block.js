...
const Imp = observer(props => {
  ...
  return <div>
    <input style={{ width: 400 }} ref={inputRef} onKeyDown={props.onKeyDown} onChange={onChange} value={value} />
    </div>
})
...
const Block = observer(props => {  
  ...
  const _handleMenuAction = ({ action, data }) => {   
    switch (action) {
        case 'setTextType':
            ...
            break
        default:
            props.onHandleMenuAction && props.onHandleMenuAction({ id: props?.blockId, action, data })
    }
  }
  ...
  const _handleKeyDownOnChildComponent = async e => {
        ...
        switch (e.key) {
            case 'Backspace':
                ...
                break
            case 'Enter':
                if (!e.ShiftKey) props.onReturnKeyPressed && props.onReturnKeyPressed(props?.blockId)
                break
            default:
                break
        }
  ...
  const _onHover = (hover, blockId) => {
    if (hover && props.store.hoveredBlockId !== blockId) props.store.hoveredBlockId = blockId//_setHoveredBlockId(blockId)
    else if (!hover && props.store.hoveredBlockId === blockId) props.store.hoveredBlockId = undefined //_setHoveredBlockId(undefined)*/
  }
  ...
  const shouldShowBlockTypeSelector = () => props.store?.findBlockInCurrentPage(props?.blockId)?.type === undefined
  ...
  return props?.blockId ? <div key={props?.blockId} className='block' onClick={e => _onClick(props?.blockId, e)}>
    <HoverableComponent onHover={hover => _onHover(hover, props?.blockId)}>
      <BlockSelector key={'selector_' + props?.blockId}
    ...
    >
        <div onClick={e => _onClick(props?.blockId, e)}>
          {Control({
            ...
            onChange: content => _saveBlockValue(props?.blockId, content?.value),
            value: block.content,
    ...
    <div style={{ display: 'none' }} key={props?.blockId}>{JSON.stringify(block?.content)}</div>
  </div> : null
})

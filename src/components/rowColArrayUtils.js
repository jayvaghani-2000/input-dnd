...
for (let i = 0; i < arr.length; i++) {
  s+='['
  for (let j = 0; j < arr[i].length; j++) {
    s += '('+i+'/'+arr[i][j].row+',' +j+'/' + arr[i][j].col+','+ (arr[i][j].content?.toString() || arr[i][j].id)+', dummy: ' + arr[i][j].isDummy + ')'
  }
  s+='], \n'
}
return s
...
for (let i = 0; i < arr.length; i++) {
  for (let j = 0; j < arr[i].length; j++) {
    if(arr[i][j].id?.toString() === id?.toString()) return {row:i, col:j}
  }
}
return undefined
}
...
const ordered = getColOrderedArray([...def].filter(block => block.row === row+1))
...
const rowColArray = { toString, moveItemToRow, findItem, getNextRowBlock, flattenArray, getRowOrderedArray, removeItemByCoordinates, removeItemById, calcRowColPositionsInArray, findItemCoordinates, getRowOrderedColOrderedArray }

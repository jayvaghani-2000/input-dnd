...
const login = async (name, pass) => {
    return await _secureCallAPI(async () => {
        ...
        // default user
            user = {
                id: userId, lastName: 'reyes', fistName: 'Patrick',
...
const fetchWorkspace = async (userId) => {
    return await _secureCallAPI(async () => await cacheManager.fetchWorkspace(userId))
}

const saveWorkspace = async (workspace, userId) => {
    return await _secureCallAPI(async () => cacheManager.saveWorkspace(workspace, userId))
}
...
const deletelock = async (page, block) => {
    return await _secureCallAPI(async () => cacheManager.deleteBlock(page, block))
}
...
export default {fetchTableData, login, saveBlock, deletelock, savePage, fetchWorkspace, saveWorkspace, saveUser, fetchPage}
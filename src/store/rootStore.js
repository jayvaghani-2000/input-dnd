...
const createNewEmptyBlock = (content, type, row, col) => ({
  id: uuidv4(),
  content,
  type,
  row,
...
const removeDummyBlock = (page, dummyBlockId) => ({
  ...page,
  blocks: page.blocks.filter((id) => id !== dummyBlockId),
});
...
class rootStore {
  ...
  login = async (name, password) => {
    ...
    if (user !== undefined) {
      ...
      if (!workspace) {
        ...
        const nextNum =
          workspace.pages.filter((page) => page.title.indexOf(noTitle) > -1)
            .length + 1;
        ...
        if (!workspace.pages.find((p) => p.id === pageToFetchId)) {
          ...
          pageToFetchId = workspace.pages.find(
            (p) => !p.isDeleted && !p.isTemplate
          )?.id;
          if (pageToFetchId)
            pageDefinition = await this._fetchAndSetAsCurrentPage(
              pageToFetchId
            );
          else {
    ...
    if (!pageDefinition) {
      ...
      if (pageDefinition?.blocksInError?.length > 0) {
        ...
        console.warn(
          "Page " +
            pageToFetchId +
            " was fetched from cache but contains missing blocks "
        );
      }
    }
  ...
  insertBlockAbove = async (id) => {
    ...
    let arr = rowColArray.getRowOrderedColOrderedArray(
  ...
  canMoveRight = (id) => {
    ...
    return (
      block?.col <
        rowColArray.getRowOrderedColOrderedArray(this.blocksForCurrentPage)[
          block.row
        ]?.length -
          1 && !block.isDummy
    );
  ...
  canMoveDown = (id) => {
    ...
    return (
      block?.row <
        Math.max(...this.blocksForCurrentPage.map((b) => b.row)) - 1 ||
      (rowColArray.getRowOrderedColOrderedArray(this.blocksForCurrentPage)[
        block.row
      ]?.length > 1 &&
  ...
  updatePage = async (obj) => {
    ...
    const index = this.workspace.pages.findIndex(
  ...
  getSeconds = (_) =>
    this.secondsPassed.length > 0
      ? this.secondsPassed.find((block) => block.id === 1).count
      : 0;
  findBlockInCurrentPage = (blockId) =>
    this.blocksForCurrentPage.find((block) => block.id === blockId);
  findBlockIndexInCurrentPage = (blockId) =>
    this.blocksForCurrentPage.findIndex((block) => block.id === blockId);
  setFocusedBlockId = (blockId) => (this.focusedBlockId = blockId);
  setCurrentFocusedBlock = (blockId) => {
    this.currentFocusedBlock = blockId;
  };
  setCurrentHoveredBlock = (blockId) => {
    this.currentHoveredBlock = blockId;
  };
  resetBlockToDefaultType = async (blockId) => {
    await this.updateBlock(blockId, "", "none");
  };
  _getMaxRows = (_) =>
    this.blocksForCurrentPage && this.blocksForCurrentPage.length > 0
      ? Math.max(...(this.blocksForCurrentPage || []).map((b) => b.row)) + 1
      : 0;
  ...
  addNewPage = async (_) => {
    const noTitle = "no title";
    const nextNum =
      this.workspace.pages.filter((page) => page.title.indexOf(noTitle) > -1)
        .length + 1;
    const newPage = this._addNewPage(noTitle + " " + nextNum);
  ...
  initCurrentPage = () => {
    ...
    if (
      ...
      !this.findBlockInCurrentPage(lastBlockId)?.isDummy
    )
      this._addDummyBlockAtEndOfPage();
  };
  updateBlock = async (
    blockId,
    content,
  ...
  _storeCurrentPageToCache = async (_) => {
    if (!this.currentPage) {
      console.error("Could not store undefined page");
      return;
    }
    const dummyBlockId = this.blocksForCurrentPage.find(
      (block) => block.isDummy
    )?.id;
    await api.savePage(
      removeDummyBlock(this.currentPage, dummyBlockId),
      this.blocksForCurrentPage
    );
  };
  ...
  fetchPageAndDependencies_old = (pageId, createIfNotFound) => {
    ...
    if (!pageDefinition) {
      ...
      // into local cache
      if (!createIfNotFound)
        console.log("Could not fetch page " + pageId + " from cache");
      else {

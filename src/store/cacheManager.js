...
const cacheManager = {
    ...
    saveWorkspace: (workspace, userId) => {
        window.localStorage.setItem('workspace_'+ userId, JSON.stringify(workspace))
    },
    saveBlock: (page, block) => {
        ...
        window.localStorage.setItem(`blocksForCurrentPage_${pageId}_${block.id}`, JSON.stringify(block))
    },
    ...
    savePage: (page, blocksMap=[]) => {
        ...
        const blocksToSave = blocksMap.filter(block => page.blocks.includes(block.id))
        blocksToSave.forEach(block => window.localStorage.setItem(`blocksForCurrentPage_${page.id}_${block.id}`,
    ...
    fetchPage: pageId => {
        ...
        if (!page || !page.blocks || !page.id) {
            console.warn('page ' + pageId + ' is missing or could not be deserialised')
            return
        }
        ...
        page.blocks?.forEach(blockId => {
            const blockstring = window.localStorage.getItem(`blocksForCurrentPage_${pageId}_${blockId}`)
            if (!blockstring) {
                blocksInError.push(blockId)
                console.warn('block ' + blockId + ' is missing in cache for page ' + pageId)
            } else {
                try {
                    pageBlocks.push(JSON.parse(blockstring))
                }
                catch (e) {
                    console.warn(`Could not deserialise block ${pageId}_${blockId} from cache`)
                    blocksInError.push(blockId)
                }
            }
        })
        ...
        return {
            currentPage: { ...page, blocks: page.blocks.filter(blockId => !blocksInError.includes(blockId)) },
